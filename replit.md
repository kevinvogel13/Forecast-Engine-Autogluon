...
- 2026-04-04: Ninth audit pass — fixed 3 bugs: (1) `forecastChartData` forecast row time-key detection picked `item_id` before `timestamp` for AutoGluon rows — fixed by explicitly preferring `'timestamp'` when present in the row; also excluded `item_id`, `mean`, and numeric-quantile keys (e.g. `'0.1'`) from the candidate list; (2) same robust time-key logic applied to the backtest row detection — explicitly prefers `'timestamp'`, then falls back to first key that excludes `_id`-suffix and uppercase-start patterns; (3) `totalForecast` fell back to `0` when AutoGluon used `0.5` quantile column instead of `mean` — added `r['0.5']` as a third fallback after `r.forecast` and `r['mean']`.
- 2026-04-04: Eighth audit pass — fixed 3 bugs: (1) merge handler used `leftColumn`/`rightColumn` (singular strings) but frontend saves `leftKeys`/`rightKeys` (arrays) — join keys were silently ignored; fixed to read array format with singular fallback; (2) pivot handler read `pivotType` but frontend saves `pivotMode`; unpivot read `unpivotIdCols`/`unpivotValueCols` but frontend saves `unpivotIdColumns`/`unpivotValueColumns` — both operations were broken; fixed with fallback chain; (3) `ForecastResultsDashboard` chart time key detection attempted auto-detection but for AutoGluon output this produced `item_id` as the time axis — partial fix noted (full fix in ninth pass).
