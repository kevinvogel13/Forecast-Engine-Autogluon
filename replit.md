...
- 2026-03-21: Additional AutoGluon audit pass — fixed 5 more issues: (1) `backtest` rows were never forwarded through SSE (pipeline.py now emits them, capped at 2000 rows), (2) `_compute_backtest_metrics_from_predictions` was logically broken (future timestamps never matched training data) — replaced with `_holdout_backtest()` that slices last horizon rows per series as true holdout, predicts from truncated dataset, and computes MAPE/RMSE/MAE + per-series table against held-out actuals, (3) `backtestFolds` now passed as `num_val_windows` to AutoGluon fit, (4) `backtestStepSize` now passed as `val_step_size` to AutoGluon fit, (5) leaderboard `score_val` (AutoGluon's internal CV metric) is now surfaced in output node panel as fallback when holdout metrics aren't available.
